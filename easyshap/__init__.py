...
def eshap_compare(
    ...
    """
    # todo: maybe it would be better to return a xr.DataSet so that we can store the explanations aswell, not just the difference
    ...
    if output_names is None:
        output_names = [f"output_{i}" for i in range(len(shap_values1))]
    if isinstance(shap_values1, list) and isinstance(shap_values2, list):
        ...
        if len(shap_values1) == 1 and len(shap_values2) == 1:
            shap_values1_xarr = xr.DataArray(
                shap_values1[0], dims=data.dims, coords=data.coords
            )
            shap_values2_xarr = xr.DataArray(
                shap_values2[0], dims=data.dims, coords=data.coords
            )
        else:
            shap_values1_xarr = xr.DataArray(
                shap_values1,
        ...
        shap_values1_xarr = xr.DataArray(
            shap_values1, dims=data.dims, coords=data.coords
        )
        shap_values2_xarr = xr.DataArray(
            shap_values2, dims=data.dims, coords=data.coords
        )
    return shap_values1_xarr - shap_values2_xarr
